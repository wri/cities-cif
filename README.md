...
2. The spatial extent of the requested layer AOI can be specified through the GeoExtent class in two ways for an AOI:
   ...
   2. the city_id and area_of_interest_id as a json snippet, such as:
      '''
      {"city_id": "BRA-Florianopolis", "area_of_interest": "city_admin_level"}
      '''
      * Note: the city_id and area_of_interest may be determined via the Cities Indicators API at https://dev.cities-data-api.wri.org/docs
