...
def test_area_fractional_vegetation_exceeds_threshold__percent():
    ...
    assert_metric_stats(indicator, 2, 92.35, 96.26, 3, 0)
...
def test_built_land_with_high_lst__percent():
    ...
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 1, 9.9, 11.4, 100, 0)
...
def test_built_land_with_vegetation__percent():
    ...
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 1, 35.9, 40.1, 2, 1)
...
def test_impervious_surface_on_urbanized_land__percent():
    ...
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 2, 59.09, 100.00, 100, 0)
...
def test_number_species_bird_richness__species():
    random.seed(42)
    indicator = BirdRichness__Species().get_metric(GEOZONE_TERESINA)
    expected_zone_size = len(GEOZONE_TERESINA.zones)
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 1, 22, 69, 3, 136)

def test_number_species_arthropod_richness__species():
    random.seed(42)
    indicator = ArthropodRichness__Species().get_metric(GEOZONE_TERESINA)
    expected_zone_size = len(GEOZONE_TERESINA.zones)
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 1, 21, 145, 5, 134)

def test_number_species_vascular_plant_richness__species():
    random.seed(42)
    indicator = VascularPlantRichness__Species().get_metric(GEOZONE_TERESINA)
    expected_zone_size = len(GEOZONE_TERESINA.zones)
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 1, 26.0, 26.0, 1, 39)
...
def test_recreational_space_per_thousand__hectaresperthousandpersons():
    ...
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 2, 0, 0.46, 100, 0)
...
def test_riparian_land_with_vegetation_or_water__percent():
    ...
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 2, 92.04, 96.27, 3, 0)
...
def test_steeply_sloped_land_with_vegetation__percent():
    ...
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 2, 88.24, 100, 2, 1)
...
def test_tree_carbon_flux__tonnes():
    ...
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 2, -73.60, -0.53, 100, 0)
...
def test_urban_open_space__percent():
    ...
    actual_indicator_size = len(indicator)
    assert expected_zone_size == actual_indicator_size
    assert_metric_stats(indicator, 3, 0, 3.177, 100, 0)
