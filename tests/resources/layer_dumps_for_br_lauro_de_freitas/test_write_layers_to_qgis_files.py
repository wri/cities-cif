...
from city_metrix.layers import (
    ...
    TreeCover,
    UrbanLandUse,
    WorldPop, Layer, EsaWorldCoverClass
)
...
@pytest.mark.skipif(RUN_DUMPS == False, reason='Skipping since RUN_DUMPS set to False')
def test_write_built_up_vegetation(target_folder, bbox_info, target_spatial_resolution_multiplier):
    file_path = prep_output_path(target_folder, 'built_up_vegetation.tif')
    target_resolution = target_spatial_resolution_multiplier * get_class_default_spatial_resolution(BuiltUpVegetation())

    built_up_class = EsaWorldCoverClass.BUILT_UP
    BuiltUpVegetation(land_cover_class=built_up_class, spatial_resolution=target_resolution).write(bbox_info.bounds, file_path, tile_degrees=None)
    assert verify_file_is_populated(file_path)
