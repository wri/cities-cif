...
from tests.resources.tools import get_test_bbox
from ..bbox_constants import GEOEXTENT_SMALL_CITY_WGS84, GEOEXTENT_SMALL_CITY_UTM
...
def test_esa_world_cover_os(target_folder):
    ...
    bbox = get_test_bbox(GEOEXTENT_SMALL_CITY_WGS84)
...
def test_write_nasa_dem_utm(target_folder):
    ...
    bbox = get_test_bbox(GEOEXTENT_SMALL_CITY_UTM)
    NasaDEM().write(bbox, file_path)
    assert verify_file_is_populated(file_path)
