# File defines bboxes using in the test code
from city_metrix.layers.layer import WGS_CRS
from city_metrix.layers.layer_geometry import LayerBbox

BBOX_BRA_LAURO_DE_FREITAS_1 = LayerBbox(
    bbox=(-38.35530428121955,-12.821710300686393,-38.33813814352424,-12.80363249765361), crs=WGS_CRS
)

BBOX_BRA_SALVADOR_ADM4 = LayerBbox(
    bbox=(-38.647320153390055,-13.01748678217598787,-38.3041637148564007,-12.75607703449720631), crs=WGS_CRS
)

# UTM Zones 22S and 23S
BBOX_BRA_BRASILIA = LayerBbox(
    bbox=(-48.07651,-15.89788, -47.83736,-15.71919), crs=WGS_CRS
)

BBOX_SMALL_TEST = LayerBbox(
    bbox=(-38.43864,-12.97987,-38.39993,-12.93239), crs=WGS_CRS
)

BBOX_NLD_AMSTERDAM_TEST = LayerBbox(
    bbox=(4.9012,52.3720,4.9083,52.3752), crs=WGS_CRS
)

BBOX_NLD_AMSTERDAM_LARGE_TEST = LayerBbox(
    bbox=(4.884629880473071,52.34146514406914,4.914180290924863,52.359560786247165), crs=WGS_CRS
)
