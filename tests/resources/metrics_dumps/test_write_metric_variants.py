...
def test_write_large_city(target_folder):
    ...
    NaturalAreas__Percent().write_as_geojson(ZONE, s3_env=DEFAULT_DEVELOPMENT_ENV, output_uri=file_path)
...
def test_write_data_series_as_geojson(target_folder):
    ...
    BuiltLandWithHighLST__Percent().write_as_geojson(zones, s3_env=DEFAULT_DEVELOPMENT_ENV, output_uri=file_path)
    assert verify_file_is_populated(file_path)
    cleanup_cache_files(None, None, None, file_path)
...
def test_write_data_series_as_csv(target_folder):
    ...
    BuiltLandWithHighLST__Percent().write(zones, s3_env=DEFAULT_DEVELOPMENT_ENV, output_uri=file_path)
    assert verify_file_is_populated(file_path)
    cleanup_cache_files(None, None, None, file_path)
