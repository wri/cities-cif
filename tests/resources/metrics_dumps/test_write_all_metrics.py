...
def test_write_era_5_met_preprocessingUmep(target_folder):
    ...
    file_path = prep_output_path(
        target_folder, 'metric', 'era_5_met_preprocessing_umep.csv')
...
@pytest.mark.skipif(DUMP_RUN_LEVEL != DumpRunLevel.RUN_FAST_ONLY, reason=f"Skipping since DUMP_RUN_LEVEL set to {DUMP_RUN_LEVEL}")
def test_write_era_5_met_preprocessingUPenn(target_folder):
    zones = SAMPLE_TILED_SINGLE_ZONE
    file_path = prep_output_path(
        target_folder, 'metric', 'era_5_met_preprocessing_upenn.csv')
