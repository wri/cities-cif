...
class LandSurfaceTemperature:
    ...
    def extract_gee(self, city: City):
        ...
        #
        # boundary_uri = f"s3://cities-indicators/data/boundaries/boundary-{city}-ADM{admin_level}"
        # boundaries = gpd.read_file(f"{boundary_uri}.geojson").reset_index()
        # boundaries_union = gpd.read_file(f"{boundary_uri}union.geojson").reset_index()
        #
        # # boundary_geo = boundaries_union.to_json()
        # boundary_geo = json.loads(boundaries_union.to_json())
