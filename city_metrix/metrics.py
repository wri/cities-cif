...
def built_land_without_tree_cover(zones):
    ...
    tree_cover_in_built_land = tree_cover.mask(built_up_land).zones(zones).count()
...
def surface_reflectivity(zones, start_time, end_time, albedo_threshold):
    ...
    built_albedo_counts = albedo.mask(built_up_land).zones(zones).count()
...
def high_land_surface_temperature(zones):
    ...
    built_land_counts = built_up_land.groupby(zones).count()
    built_high_lst_counts = high_lst.mask(built_up_land).zones(zones).count()
