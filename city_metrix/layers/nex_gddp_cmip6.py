...
def get_var(varname, model, latlon, start_year=HIST_START, end_year=HIST_END, yearshift=False, scenario='ssp585'):
    if varname=='hurs' and model in HURS_EXCLUDED:
        raise Exception(f'Model {model} does not include complete data for hurs')
...
def get_best_models(varname, latlon, hist_start, hist_end, num_bestmodels):
    ...
    hist_obs = get_var(varname, 'ERA5', latlon, start_year=hist_start, end_year=hist_end)
    hist_mods = {}
    rmsds = []
    if varname == 'hurs':
        ...
        models = MODELS
    for model in models:
        hist_mod = get_var(varname, model, latlon, start_year=hist_start, end_year=hist_end)
