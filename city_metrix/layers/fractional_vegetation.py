...
class FractionalVegetation(Layer):
    ...
    def get_data(self, bbox: GeoExtent, spatial_resolution:int=DEFAULT_SPATIAL_RESOLUTION, resampling_method=None):
        ...
        def calcFr(aoi, vegpctl, soilpctl):
            ...
            # Cloud score+
            S2filtered = S2.filterBounds(aoi).filterDate(date_start, date_end).linkCollection(S2CS, ['cs']).map(lambda img: img.updateMask(img.select('cs').gte(CLEAR_THRESHOLD)).divide(10000))
            ...
            dwClass = dwMode.remap([0,1,2,3,4,5,6,7,8], [0,1,1,0,0,1,2,0,0]).selfMask().rename('lc')
            ...
            vegNDVI = ndvi.updateMask(dwClass.eq(1)).reduceRegion(
                reducer = ee.Reducer.percentile([PCTL_FULLVEG]),
                geometry = aoi,
                scale = 10,
                maxPixels = 10e13
                ).get('NDVI')
            soilNDVI = ndvi.updateMask(dwClass.eq(2)).reduceRegion(
