...
class LandCoverGlad(Layer):
    ...
    def get_data(self, bbox: GeoExtent, spatial_resolution:int=DEFAULT_SPATIAL_RESOLUTION,
        ...
        lcluc_ic = ee.ImageCollection(ee.Image(f'projects/glad/GLCLU2020/v2/LCLUC_{self.year}'))
...
class LandCoverSimplifiedGlad(Layer):
    ...
    def get_data(self, bbox: GeoExtent, spatial_resolution:int=DEFAULT_SPATIAL_RESOLUTION,
        ...
        # data = data.where((glad < 1) | (glad > 24), 1)  # short vegetation
        ...
        # ARD legend here: https://storage.googleapis.com/earthenginepartners-hansen/GLCLU2000-2020/v2/legend.xlsx
        data = data.where(glad != 254, 10)                  # open water (ocean)
        data = data.where((glad < 250) | (glad > 253), 8)   # builtup
        data = data.where((glad < 249) | (glad > 244), 7)   # crop
        data = data.where((glad < 243) | (glad > 241), 9)   # barren (snow / ice)
        data = data.where(glad != 240, 3)                   # short vegetation
        data = data.where((glad < 211) | (glad > 208), 10)  # open water
        data = data.where((glad < 125) | (glad > 196), 6)   # wetland trees
        data = data.where((glad < 102) | (glad > 124), 5)   # wetland short veg
        data = data.where((glad < 100) | (glad > 101), 9)   # barren (salt pan)
        data = data.where((glad < 25) | (glad > 96), 4)     # trees
        data = data.where((glad < 19) | (glad > 24), 3)     # short vegetation
        data = data.where((glad < 2 ) | (glad > 18), 2)      # semi-arid
        data = data.where((glad < 0) | (glad > 1), 1)       # desert

        return data.where(data <= 10, np.nan)
...
class LandCoverHabitatGlad(Layer):
    ...
    Attributes:
        year: year used for data retrieval
    """
    def __init__(self, year=2020, return_value=None, **kwargs):
        super().__init__(**kwargs)
        self.year = year
    ...
    def get_data(self, bbox: GeoExtent, spatial_resolution:int=DEFAULT_SPATIAL_RESOLUTION,
        ...
        # result = xr.where(simplified_glad <= 6, 1, 0)
        # result.rio.write_crs(bbox.as_utm_bbox().crs, inplace=True)
        # return result
        result = xr.full_like(simplified_glad, 0)
        result = result.where(simplified_glad > 6, other=1)

        if self.return_value is not None:
            result = result.where(result==self.return_value, np.nan)
...
class LandCoverHabitatChangeGlad(Layer):
    ...
    """
    def __init__(self, start_year=2000, end_year=2020, return_value=None, **kwargs):
        super().__init__(**kwargs)
    ...
    def get_data(self, bbox: GeoExtent, spatial_resolution:int=DEFAULT_SPATIAL_RESOLUTION,
        ...
        if self.return_value is not None:
            habitatchange = habitatchange.where(habitatchange==self.return_value, other=np.nan)
