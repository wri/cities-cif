...
MAX_RASTER_BYTES_FOR_SINGLE_FILE_OUTPUT = (
    500  # 500000000 # (Note: Teresina FabDem 544997973)
)
...
class GeoZone:
    def __init__(self, geo_zone: Union[GeoDataFrame | str], crs=WGS_CRS):
        if isinstance(geo_zone, str):
            ...
            elif ("city_admin_level" in geo_zone.lower()) or (
                "urban_extent" in geo_zone.lower()
            ):
                self.geo_type = GeoType.CITY_AREA
            else:
                raise Exception(
                    "Invalid aoi_id, should be 'city_centroid', 'urban_extent' or 'city_admin_level'"
                )
        else:
        ...
        if self.geo_type == GeoType.GEOMETRY:
            ...
            if self.geo_type == GeoType.CITY_AREA:
                ...
                self.bbox, self.crs, self.zones = _build_aoi_from_city_boundaries(
                    self.city_id, self.admin_level
                )
...
def _build_aoi_from_city_boundaries(city_id, geo_feature):
    ...
    reproj_west, reproj_south, reproj_east, reproj_north = boundaries_gdf.to_crs(
        utm_crs
    ).total_bounds

    # Round coordinates to whole units
    bbox = (
        math.floor(reproj_west),
        math.floor(reproj_south),
        math.ceil(reproj_east),
        math.ceil(reproj_north),
    )
...
class GeoExtent:
    def __init__(
        self,
        bbox: Union[tuple[float, float, float, float] | GeoZone | str],
        crs=WGS_CRS,
    ):
    ...
    def to_ee_rectangle(self):
        ...
        if self.projection_type == ProjectionType.GEOGRAPHIC:
            ...
            ee_rectangle = GeoExtent._build_ee_rectangle(
                minx, miny, maxx, maxy, utm_bbox.crs
            )
    ...
    def _build_ee_rectangle(min_x, min_y, max_x, max_y, crs):
        ...
        ee_rectangle = ee.Geometry.Rectangle(
            [buf_minx, buf_miny, buf_maxx, buf_maxy], crs, geodesic=False
        )
    ...
    def as_utm_bbox(self):
        ...
        if self.projection_type == ProjectionType.GEOGRAPHIC:
            ...
            if self.geo_type == GeoType.CITY_AREA:
                ...
                reproj_bbox = reproject_units(
                    self.min_y, self.min_x, self.max_y, self.max_x, WGS_CRS, utm_crs
                )
    ...
    def as_geographic_bbox(self):
        ...
        if self.projection_type == ProjectionType.GEOGRAPHIC:
            ...
            reproj_bbox = reproject_units(
                self.min_x, self.min_y, self.max_x, self.max_y, self.crs, WGS_CRS
            )
            # round to minimize drift
            rounded_box = (
                reproj_bbox[0],
...
def _truncate_to_nearest_interval(tile_side_meters, spatial_resolution):
    ...
    floor_increment = (
        math.floor((tile_side_meters / spatial_resolution)) * spatial_resolution
    )
    ceil_increment = (
        math.ceil((tile_side_meters / spatial_resolution)) * spatial_resolution
    )

    floor_offset = abs(floor_increment - tile_side_meters)
    ceil_offset = abs(ceil_increment - tile_side_meters)
    nearest_increment = (
        floor_increment if floor_offset < ceil_offset else ceil_increment
    )
...
def get_degree_offsets_for_meter_units(bbox: GeoExtent, tile_side_degrees):
    ...
    x_offset = get_haversine_distance(
        mid_x, bbox.min_y, mid_x + tile_side_degrees, bbox.min_y
    )

    mid_y = (bbox.min_y + bbox.max_y) / 2
    y_offset = get_haversine_distance(
        bbox.min_x, mid_y, bbox.min_x, mid_y + tile_side_degrees
    )
...
def _get_offsets(
    ...
    if output_as == ProjectionType.GEOGRAPHIC:
        if length_units == "degrees":
            ...
            raise Exception(
                "Currently does not support length_units in meters and output_as latlon"
            )
    else:  # projected
        if length_units == "degrees":
            x_tile_side_meters, y_tile_side_meters = get_degree_offsets_for_meter_units(
                bbox, tile_side_length
            )
            x_tile_side_units = _truncate_to_nearest_interval(
                x_tile_side_meters, spatial_resolution
            )
            y_tile_side_units = _truncate_to_nearest_interval(
                y_tile_side_meters, spatial_resolution
            )
            ...
            tile_side_meters = _truncate_to_nearest_interval(
                tile_side_length, spatial_resolution
            )
    ...
    if output_as == ProjectionType.GEOGRAPHIC:
        ...
        if bbox.projection_type == ProjectionType.GEOGRAPHIC:
            ...
            start_x_coord, start_y_coord = (
                math.floor(bbox.min_x),
                math.ceil(bbox.min_y),
            )
...
def create_fishnet_grid(
    bbox: GeoExtent,
    tile_side_length: float = 0,
    tile_buffer_size: float = 0,
    length_units: str = "meters",
    spatial_resolution: int = 1,
    output_as: ProjectionType = ProjectionType.UTM,
) -> gpd.GeoDataFrame:
    """
    ...
    if length_units is None:
        if tile_side_length > 0 or tile_buffer_size > 0:
            raise Exception(
                "Length_units cannot be None if tile_side_length or tile_buffer_size are > 0"
            )
        else:
    ...
    if length_units not in valid_length_units:
        raise ValueError(
            f"Invalid length_units ('{length_units}'). "
    ...
    x_tile_side_units, y_tile_side_units, tile_buffer_units = _get_offsets(
        bbox,
        tile_side_length,
        tile_buffer_size,
        length_units,
    ...
    start_x_coord, start_y_coord, end_x_coord, end_y_coord = _get_bounding_coords(
        bbox, output_as
    )
    ...
    if (
        bbox.projection_type == ProjectionType.GEOGRAPHIC
        and output_as == ProjectionType.GEOGRAPHIC
    ):
        grid_crs = WGS_CRS
    elif (
        bbox.projection_type == ProjectionType.GEOGRAPHIC
        and output_as == ProjectionType.UTM
    ):
...
class LayerGroupBy:
    ...
    def _compute_statistic(self, stats_func):
        return self._zonal_stats(
            stats_func,
            self.geo_zone,
            self.aggregate,
            self.layer,
            self.masks,
            self.custom_tile_size_m,
    ...
    def _zonal_stats(
        ...
        if box_area <= tile_size_meters**2:
            ...
            stats = LayerGroupBy._zonal_stats_fishnet(
                stats_func,
                geo_zone,
                zones,
                aggregate,
    ...
    def _zonal_stats_fishnet(
        ...
        tile_stats = pd.concat(
            [
                LayerGroupBy._zonal_stats_tile(
                    tile_funcs,
        ...
        aggregated = tile_stats.groupby("zone", as_index=False).apply(
            LayerGroupBy._aggregate_stats, stats_func
        )
    ...
    @staticmethod
    def _zonal_stats_tile(
        stats_func, tile_gdf, zones, aggregate, layer, masks, spatial_resolution
    ):
        bbox = GeoExtent(tile_gdf)

        aggregate_data = aggregate.retrieve_data(
            bbox=bbox,
            s3_bucket=CIF_CACHE_S3_BUCKET_URI,
            s3_env=DEFAULT_PRODUCTION_ENV,
            spatial_resolution=spatial_resolution,
        )
        ...
        if isinstance(aggregate_data, xr.DataArray) and aggregate_data.data.size == 0:
            ...
            if layer is not None:
                layer_data = layer.retrieve_data(
                    bbox=bbox,
                    s3_bucket=CIF_CACHE_S3_BUCKET_URI,
                    s3_env=DEFAULT_PRODUCTION_ENV,
                    spatial_resolution=spatial_resolution,
            ...
            raster_data = [
                data
                for data in mask_datum + [aggregate_data] + [layer_data]
                if isinstance(data, xr.DataArray)
            ]
            align_to = sorted(
                raster_data, key=lambda data: data.size, reverse=True
            ).pop()
            aligned_aggregate_data = LayerGroupBy._align(aggregate_data, align_to)
            aligned_mask_datum = [
                LayerGroupBy._align(data, align_to) for data in mask_datum
            ]
            ...
            if "geo_level" not in zones.columns:
                result_stats = LayerGroupBy._compute_zonal_stats(
                    stats_func,
                    layer,
                    tile_gdf,
                    align_to,
                ...
                for index, level in enumerate(geo_levels):
                    ...
                    if result_stats is None:
                        ...
                        merged_df = pd.merge(
                            result_stats, stats, on="zone", how="outer"
                        )

                        for func in stats_func:
                            func_x_col = f"{func}_x"
                            func_y_col = f"{func}_y"
                            merged_df[func] = merged_df[func_x_col].combine_first(
                                merged_df[func_y_col]
                            )
    ...
    def _compute_zonal_stats(
        stats_func, layer, tile_gdf, align_to, layer_data, aggregate_data
    ):
    ...
    def _rasterize(gdf, snap_to: xr.DataArray):
        ...
        if gdf.empty:
            ...
            shapes = [
                (geom, value)
                for geom, value in zip(reproj_gdf.geometry, reproj_gdf.index)
            ]
...
class Layer:
    ...
    def filename(self, bbox: GeoExtent) -> tuple:
    ...
    def retrieve_data(
        self,
        bbox: GeoExtent,
        s3_bucket: str = CIF_CACHE_S3_BUCKET_URI,
        s3_env: str = DEFAULT_PRODUCTION_ENV,
        aoi_buffer_m: int = None,
        city_aoi_subarea: (float, float, float, float) = None,
        spatial_resolution: int = None,
    ) -> Union[xr.DataArray, gpd.GeoDataFrame]:
        """
        ...
        if s3_bucket is None or s3_env is None:
            ...
            standard_env = standardize_s3_env(s3_env)
            has_usable_cache = is_cache_usable(
                s3_bucket, standard_env, self.aggregate, bbox, aoi_buffer_m, None
            )

        if has_usable_cache:
            result_data, _, _ = retrieve_city_cache(
                self.aggregate,
                bbox,
                aoi_buffer_m,
                s3_bucket=s3_bucket,
            ...
            result_data = self.aggregate.get_data(
                bbox=query_geoextent, spatial_resolution=spatial_resolution
            )

            # Opportunistically cache city data
            if (
                s3_bucket is not None
                and bbox.geo_type == GeoType.CITY_AREA
                and city_aoi_subarea is None
            ):
                target_uri, _, _, _ = build_file_key(
                    s3_bucket, standard_env, self.aggregate, bbox, aoi_buffer_m
                )
                self._build_city_cache(
                    bbox, spatial_resolution, target_uri, aoi_buffer_m
                )
    ...
    def _cache_data_by_fishnet_tiles(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            while len(unretrieved_tiles) > 0 and retry_count < 3:
                ...
                for index, tile in unretrieved_tiles.iterrows():
                    task = dask.delayed(self._process_fishnet_tile)(
                        index, tile, crs, spatial_resolution, temp_dir, target_uri
                    )
                    tasks.append(task)
        ...
        if len(unretrieved_tile_records) > 0:
            errors_df = pd.DataFrame(
                unretrieved_tile_records, columns=["index", "error"]
            )
            ...
            df_joined = pd.merge(
                errors_df, incomplete_tile_geometry, on="index", how="left"
            ).reset_index(drop=True)
            df_joined = df_joined[["tile_name", "error", "geometry"]].reset_index(
                drop=True
            )
            ...
            new_fishnet_grid = fishnet.merge(
                errors_df, on="index", how="left", indicator=True
            )
    ...
    def _process_fishnet_tile(
        self, index, tile, crs, spatial_resolution, temp_dir, target_uri
    ):
        ...
        tile_data = None
        failure_message = ""
        while tile_data is None and retry_count < 5:
            failure_message = ""
            try:
                ...
                # else:
                # # ----
                tile_data = self.aggregate.get_data(
                    bbox=buffered_utm_bbox, spatial_resolution=spatial_resolution
                )
            except Exception as e_msg:
    ...
    def groupby(
        self, geo_zone, spatial_resolution=None, layer=None, custom_tile_size_m=None
    ):
        """
    ...
    def _add_tile_name_column(tile_grid):
        tile_grid["tile_name"] = tile_grid.index.to_series().apply(
            lambda x: f"tile_{str(x + 1).zfill(TILE_NUMBER_PADCOUNT)}"
        )
        return tile_grid
...
def set_resampling_for_continuous_raster(
    image: ee.Image,
    ...
    if target_resolution == default_resolution:
        ...
        if resampling_method == "nearest":
            data = image.toFloat().reproject(  # Ensure values are float in order to successfully use interpolation
                crs=crs, scale=target_resolution
            )
        else:
            if kernel_convolution is None:
                data = (
                    image.toFloat()  # Ensure values are float in order to successfully use interpolation
...
def get_image_collection(
    image_collection: ImageCollection, ee_rectangle, scale: int, name: str = None
) -> xr.DataArray:
    """
    ...
    if crs == WGS_CRS:
        raise Exception(
            "Output in geographic units is currently not supported for raster layers."
        )
...
class Metric:
    ...
    @abstractmethod
    def get_metric(
        self, geo_zone: GeoZone, spatial_resolution: int
    ...
    def write(
        ...
        **kwargs,
        ...
        indicator, feature_id = self.metric.retrieve_metric(
            geo_zone, spatial_resolution=spatial_resolution
        )
    ...
    def cache_city_metric(
        ...
        if force_data_refresh:
            ...
            has_usable_cache = is_cache_usable(
                s3_bucket, standard_env, self.metric, geo_zone, None, None
            )

        if not has_usable_cache and geo_zone.geo_type == GeoType.CITY_AREA:
            target_uri, _, feature_id, _ = build_file_key(
                s3_bucket, standard_env, self.metric, geo_zone, None
            )
            result_metric = self.metric.get_metric(
                geo_zone=geo_zone, spatial_resolution=spatial_resolution
            )

            zones = geo_zone.zones
            if (
                isinstance(result_metric, pd.DataFrame)
                and "zone" in result_metric.columns
            ):
                zones["index"] = zones["index"].astype(float)
                result_metric["zone"] = result_metric["zone"].astype(float)
                results_metric_df = pd.merge(
                    zones, result_metric, left_on="index", right_on="zone", how="left"
                )
                if "metric_id" not in results_metric_df.columns:
                    results_metric_df = results_metric_df.assign(metric_id=feature_id)
                result_metric = _standardize_city_metrics_columns(
                    results_metric_df, None
                )
    ...
    def retrieve_metric(
        ...
        if has_usable_cache:
            result_metric, feature_id, _ = retrieve_city_cache(
                self.metric,
                geo_extent=geo_zone,
                aoi_buffer_m=None,
                s3_bucket=s3_bucket,
    ...
    def _expand_empty_results_to_results_with_null_value(
        self, geo_zone: GeoZone
...
def _decide_if_write_can_be_skipped(
    feature, selection_object, output_path, s3_env
    ...
    if output_path is None or len(output_path.strip()) == 0:
        ...
        default_s3_uri, _, _, _ = build_file_key(
            s3_env, feature, selection_object, None
        )
...
def _write_grid_index_to_cache(fishnet, file_uri, crs):
    ...
    for index, tile in fishnet.iterrows():
        ...
        tile_row = {"tile_name": tile_file_name, "bbox": tile_bounds}
        tiles_metadata.append(tile_row)

    # Store CRS and file_uri at the top level
    metadata = {"crs": crs, "file_uri": file_uri, "tiles": tiles_metadata}
