...
from city_metrix.constants import WGS_CRS, GTIFF_FILE_EXTENSION, GEOJSON_FILE_EXTENSION, NETCDF_FILE_EXTENSION, \
    ProjectionType, GeoType
from city_metrix.metrix_dao import write_tile_grid, write_geotiff, write_geojson, write_netcdf, \
    write_dataset, get_city, get_city_admin_boundaries, get_city_boundary, write_csv
from city_metrix.metrix_tools import standardize_y_dimension_direction, get_projection_type, \
...
class LayerGroupBy:
    ...
    def _zonal_stats_fishnet(self, stats_func):
        ...
        tile_stats = pd.concat([
            self._zonal_stats_tile(self._merge_tile_geometry_into_query_gdf(tile_gdf), tile_funcs)
    ...
    def _merge_tile_geometry_into_query_gdf(self, tile_gdf):
        query_gdf = self.zones
