...
def extract_bbox_aoi(tif_da, bbox):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with rasterio.open(temp_file) as src:
            ...
            if "t" in getattr(tif_da, "dims", []) and "band" in query_da.dims:
                # Check if band count matches time length
                if query_da.sizes.get("band") == tif_da.sizes.get("t"):
                    query_da = query_da.rename(band="t")
                    # Reattach original time coordinates
                    query_da = query_da.assign_coords(t=tif_da["t"].values)
