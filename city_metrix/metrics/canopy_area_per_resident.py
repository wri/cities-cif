...
def canopy_area_per_resident(zones: GeoDataFrame, agesex_classes=[], height=5, informal_only=False) -> GeoSeries:
    ...
    canopy_layer = TreeCanopyHeight()
    canopy_layer_get_data = canopy_layer.get_data
    canopy_layer.get_data = lambda bbox: xr.where(canopy_layer_get_data(bbox) >= height, 1, 0)
    ...
    for rownum in range(len(zones)):  # Row by row because of memory issues
        ...
        if informal_only:
            # monkey-patch ULU get_data get only informal
            informal_layer = UrbanLandUse()
            informal_layer_get_data = informal_layer.get_data
            informal_layer.get_data = lambda bbox: xr.where(informal_layer_get_data(bbox)==3, 1, np.nan)
