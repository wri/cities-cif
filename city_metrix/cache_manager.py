...
def build_file_key(
    s3_bucket: str, output_env: str, class_obj, geo_extent, aoi_buffer_m: int = None
):
    city_id = geo_extent.city_id
    admin_level = geo_extent.admin_level

    # Construct layer filename and s3 key
    cache_folder_name, feature_id, file_format, is_custom_object = build_cache_name(
        class_obj
    )
    ...
    if s3_bucket is not None:
        file_key = get_cached_file_key(
            feature_base_class_name,
            s3_bucket,
            output_env,
            cache_folder_name,
            city_id,
        ...
        file_uri = get_cached_file_uri(
            s3_bucket, file_key, (CUSTOM_CACHED_DIFFERENTLY and is_custom_object)
        )  # False-and so all objs treated as non-custom
...
def is_cache_usable(
    ...
    geo_extent,
    aoi_buffer_m=None,
    city_aoi_subarea=None,
):
    # Function determines if the cache can be used based on some limited criteria

    file_uri, _, _, _ = build_file_key(
        s3_bucket, output_env, class_obj, geo_extent, aoi_buffer_m
    )
    ...
    if (
        city_aoi_subarea is not None
        and is_cache_object_available(file_uri)
        and file_format == GTIFF_FILE_EXTENSION
    ):
        return True
...
def retrieve_city_cache(
    ...
    file_uri, file_key, feature_id, is_custom_layer = build_file_key(
        s3_bucket, output_env, class_obj, geo_extent, aoi_buffer_m
    )
...
def build_cache_name(class_obj):
    ...
    primary_qualifiers = _build_class_name_part(
        class_obj, class_obj.MAJOR_NAMING_ATTS, []
    )
    if has_matched_cls_obj_atts:
        secondary_qualifiers = ""
    else:
        secondary_qualifiers = _build_class_name_part(
            class_obj, class_obj.MINOR_NAMING_ATTS, unmatched_atts
        )

    # Determine if request it for a CIF-non-default layer
    if (
        class_obj.MINOR_NAMING_ATTS is not None
        and unmatched_atts is not None
    ...
    date_kv_string = _build_naming_string_from_standard_parameters(
        class_obj, is_custom_object
    )
...
def _build_class_name_part(layer_obj, naming_atts, mismatched_atts):
    ...
    if not (naming_atts is None or naming_atts == ""):
        for attribute in naming_atts:
            if attribute.lower() not in DATE_ATTRIBUTES and (
                mismatched_atts == [] or attribute in mismatched_atts
            ):
...
def _build_naming_string_from_standard_parameters(feature_obj, is_custom_feature):
    ...
    feature_date_atts = {
        key: feature_atts[key] for key in DATE_ATTRIBUTES if key in feature_atts
    }
    ...
    for key, value in feature_date_atts.items():
        ...
        if date_key == "year":
            ...
            start_year_kv_string = _construct_naming_string_from_date_attribute(
                "start_year", date_value, "__"
            )
            end_year_kv_string = _construct_naming_string_from_date_attribute(
                "end_year", date_value, "_"
            )
            ...
            if key in ("end_year", "end_date") and has_paired_start_end_dates:
                ...
                separator = "__"
            string_val = _construct_naming_string_from_date_attribute(
                date_key, date_value, separator
            )
...
def is_cache_object_available(file_uri):
    ...
    if uri_scheme == "s3":
        ...
        file_response = s3_client.list_objects_v2(
            Bucket=s3_bucket, Prefix=file_key, MaxKeys=1
        )
        ...
        folder_response = s3_client.list_objects_v2(
            Bucket=s3_bucket, Prefix=folder_path, MaxKeys=1
        )
...
def get_cached_file_key(
    feature_based_class_name,
    s3_bucket,
    output_env,
    feature_name,
    city_id,
    ...
    if feature_based_class_name.lower() == "layer":
        ...
        file_key = (
            f"data/{output_env}/metrics/{city_id}/{city_id}{bound_marker}__{feature_id}"
        )
